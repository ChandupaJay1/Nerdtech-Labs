    ...
    <div class="col-md-8 mx-auto">
        <div class="card p-4">
            ...
            <form action="{{ route('admin.tasks.update', $task) }}" method="POST">
                ...
                @else
                <!-- Normal Admin View (Read Only details) -->
                <div class="alert alert-light border mb-4">
                    <h5 class="fw-bold">{{ $task->title }}</h5>
                    <p class="mb-1 text-muted">{{ $task->description ?? 'No description provided.' }}</p>
                    <hr>
                    <div class="d-flex justify-content-between text-sm">
                        <span><strong>Assigned By:</strong> {{ $task->assignor->name ?? 'Unknown' }}</span>
                        <span><strong>Due Date:</strong> {{ $task->due_date ? \Carbon\Carbon::parse($task->due_date)->format('M d, Y') : 'No Date' }}</span>
                    </div>
                </div>
                @endif
